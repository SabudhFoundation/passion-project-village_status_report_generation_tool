...
def get_external_village_data(village_name: str) -> str:
    ...
    try:
        ...
        # UPGRADE: A broader, more natural query that guarantees hits
        # on Wikipedia and village aggregator sites
        query = (
            f'"{village_name}" village Punjab facilities amenities '
            f'water schools healthcare census'
        )

        # Increase max_tokens to capture more webpage content
...
def generate_report_from_web(
    village_name: str,
    web_context: str,
    lang: str
) -> str:
    """
    Instructs Gemini to extract web context into strict JSON
    with explicit extraction hints.
    """
...
1. SMART EXTRACTION:
   Extract exact numbers where available.
   If missing, summarize qualitative facts.

   EXTRACTION HINTS (CRITICAL):
   - For Water/Sanitation, explicitly scan the text for words like:
     "hand pump", "tap", "well", "drinking water",
     "drainage", "toilet", "bath".

   - For Economy, scan for:
     "agriculture", "farming", "bank", "cooperative".

2. BALANCED BULLET POINTS (NO LARGE PARAGRAPHS):
   Break the information down into 3 to 5 distinct bullet points
   per domain.

   Each bullet point should be 1 to 2 sentences maximum.

3. BOLD SUB-HEADINGS:
   Every bullet point MUST start with a bolded sub-heading.

   Example:
   "**Sanitation Coverage:** 52.3% of households have toilet facilities.
   Efforts are ongoing to improve drainage."

4. Output ONLY a valid JSON object.

   Do NOT include markdown formatting outside the JSON values
   or code blocks.

5. The VALUES of the JSON must be translated into
   {'Punjabi' if lang == 'pa' else 'English'}.
...
{{
    ...
    "domains": {{
        "Basic Profile & Demographics": [
            "**Total Population:** 1-2 sentences of detailed information here.",
            "**Demographic Breakdown:** 1-2 sentences of detailed information here.",
            "**Geographic Area:** 1-2 sentences of detailed information here."
        ],

        "Health & Education": [
            "**Educational Facilities:** 1-2 sentences of detailed information here.",
            "**Healthcare Infrastructure:** 1-2 sentences of detailed information here.",
            "**Veterinary Services:** 1-2 sentences of detailed information here."
        ],

        "Water, Sanitation & Infrastructure": [
            "**Drinking Water Sources:** 1-2 sentences of detailed information here.",
            "**Sanitation Coverage:** 1-2 sentences of detailed information here.",
            "**Connectivity:** 1-2 sentences of detailed information here."
        ],

        "Governance & Economy": [
            "**Primary Economy:** 1-2 sentences of detailed information here.",
            "**Worker Demographics:** 1-2 sentences of detailed information here.",
            "**Local Administration:** 1-2 sentences of detailed information here."
        ]
    }}
}}
"""
